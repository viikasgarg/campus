...
PollAnswerFormSet = inlineformset_factory(Course, PollAnswer, extra=2, max_num=defaults.PYBB_POLL_MAX_ANSWERS,
                                          form=PollAnswerForm, formset=BasePollAnswerFormset)


class PostForm(forms.ModelForm):
    name = forms.CharField(label=ugettext_lazy('Subject'))
    poll_type = forms.TypedChoiceField(label=ugettext_lazy('Poll type'), choices=Course.POLL_TYPE_CHOICES, coerce=int)
    ...
    #Move args to kwargs
        ...
        if not (self.course or self.notice or ('instance' in kwargs)):
            raise ValueError('You should provide course, notice or instance')
            #Handle course subject, poll type and question if editing course head
        if kwargs.get('instance', None) and (kwargs['instance'].course.head == kwargs['instance']):
            kwargs.setdefault('initial', {})['name'] = kwargs['instance'].course.name
            kwargs.setdefault('initial', {})['poll_type'] = kwargs['instance'].course.poll_type
            kwargs.setdefault('initial', {})['poll_question'] = kwargs['instance'].course.poll_question

        super(PostForm, self).__init__(**kwargs)

        # remove course specific fields
        if not (self.notice or (self.instance.pk and (self.instance.course.head == self.instance))):
    ...
    def clean(self):
        ...
        if poll_type is not None and poll_type != Course.POLL_TYPE_NONE and not poll_question:
...
try:
    class EditProfileForm(forms.ModelForm):
        class Meta(object):
            model = util.get_noticeapp_profile_model()
...
class PollForm(forms.Form):
    def __init__(self, course, *args, **kwargs):
        ...
        qs = PollAnswer.objects.filter(course=course)
        if course.poll_type == Course.POLL_TYPE_SINGLE:
            self.fields['answers'] = forms.ModelChoiceField(
                label='', queryset=qs, empty_label=None,
                widget=forms.RadioSelect())
        elif course.poll_type == Course.POLL_TYPE_MULTIPLE:
    ...
    def clean_answers(self):
        ...
        if self.course.poll_type == Course.POLL_TYPE_SINGLE:
            return [answers]
        else:
            return answers
