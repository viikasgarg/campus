...
from noticeapp import util

class NoticeappMiddleware(object):
    def process_request(self, request):
        if request.user.is_authenticated():
            try:
                # Here we try to load profile, but can get error
                # if user created during syncdb but profile model
                # under south control. (Like noticeapp.Profile).
                profile = util.get_noticeapp_profile(request.user)
                ...
                from noticeapp.models import user_saved
