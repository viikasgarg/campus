...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for dup_group in orm['noticeapp.CourseReadTracker'].objects.values('course', 'user')\
                                                     .annotate(Count('course'), Count('user'))\
                                                     .filter(course__count__gt=1):
            for dup in orm['noticeapp.CourseReadTracker'].objects.filter(course=dup_group['course'], user=dup_group['user'])\
                                                   .order_by('-time_stamp')[1:]:
                dup.delete()

        for dup_group in orm['noticeapp.NoticeReadTracker'].objects.values('notice', 'user')\
                                                     .annotate(Count('notice'), Count('user'))\
                                                     .filter(notice__count__gt=1):
            for dup in orm['noticeapp.NoticeReadTracker'].objects.filter(course=dup_group['notice'], user=dup_group['user'])\
                                                   .order_by('-time_stamp')[1:]:
                dup.delete()
