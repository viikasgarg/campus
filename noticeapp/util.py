...
from noticeapp.compat import get_username_field, get_user_model
...
def build_cache_key(key_name, **kwargs):
    if key_name == 'anonymous_course_views':
        return 'noticeappm_anonymous_course_%s_views' % kwargs['course_id']
    else:
...
class FilePathGenerator(object):
    ...
    def deconstruct(self, *args, **kwargs):
        return 'noticeapp.util.FilePathGenerator', [], {'to': self.to}
