...
from localflavor.in_.models import INStateField, PhoneNumberField
...
from helper_functions import round_as_decimal
...
def create_faculty(instance, make_user_group=True):
    """ Create a Faculty object that is linked to the given
...
def create_student(instance):
    """ Create a Student object that is linked to the given auth_user
...
if 'south' in settings.INSTALLED_APPS:
    from south.modelsinspector import add_introspection_rules
    add_introspection_rules([], ["^profiles\.models\.IntegerRangeField"])
...
class Student(User, CustomFieldModel):
    ...
    state = INStateField(blank=True, editable=False, null=True)
