...
from profiles.admin import StudentFileInline
...
from work_study.forms import StudentForm, WorkTeamForm
from profiles.helper_functions import ReadPermissionModelAdmin
...
class CompanyAdmin(admin.ModelAdmin):
    def render_change_form(self, request, context, *args, **kwargs):
        if 'original' in context and context['original']:
            ...
            txt += '<a href="{0}">{0}</a>'.format(settings.BASE_URL + reverse('work_study.views.company_contract1', args=(context['original'].id,)))
...
class TimeSheetAdmin(admin.ModelAdmin):
    def render_change_form(self, request, context, *args, **kwargs):
        if 'original' in context:
            ...
            from django.conf import settings
            from django.core.urlresolvers import reverse
            from work_study.views import approve
