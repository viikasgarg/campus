...
from profiles.helper_functions import round_as_decimal
...
class CourseMeet(models.Model):
    ...
    def __unicode__(self):
        return "%s [%s-%s] %s" % (self.course.shortname, self.start_time.strftime('%I:%M%p'), self.end_time.strftime('%I:%M%p'), self.location.name)
...
class CourseEnrollment(models.Model):
    ...
    year = models.ForeignKey('profiles.GradeLevel', blank=True, null=True)
...
class DepartmentGraduationCredits(models.Model):
    ...
    class_year = models.ForeignKey('profiles.ClassYear', help_text='Also applies to subsequent years unless a more recent requirement exists.')
    credits = models.DecimalField(max_digits=5, decimal_places=2)
    class Meta:
        unique_together = ('department', 'class_year')

class Course(models.Model):
    active = models.BooleanField(default=True, help_text="Sometimes used in third party integrations such as Moodle.")
    ...
    teacher = models.ForeignKey('profiles.Faculty', blank=True, null=True, related_name="ateacher")
    secondary_teachers = models.ManyToManyField('profiles.Faculty', blank=True, null=True, related_name="secondary_teachers")
...
class OmitYearGPA(models.Model):
    ...
    student = models.ForeignKey('profiles.Student')
    year = models.ForeignKey('profiles.SchoolYear', help_text="Omit this year from GPA calculations and transcripts.")
    def __unicode__(self):
